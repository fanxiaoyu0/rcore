...
pub fn syscall(syscall_id: usize, args: [usize; 4]) -> isize {
    ...
    match syscall_id {
        SYSCALL_LINKAT => {inner.syscall_times[SYSCALL_LINKAT]+=1;drop(inner);return sys_linkat(args[1] as *const u8, args[3] as *const u8);},
        SYSCALL_UNLINKAT => {inner.syscall_times[SYSCALL_UNLINKAT]+=1;drop(inner);return sys_unlinkat(args[1] as *const u8);},
        SYSCALL_OPEN => {inner.syscall_times[SYSCALL_OPEN]+=1;drop(inner);return sys_open(args[1] as *const u8, args[2] as u32);},
        SYSCALL_CLOSE => {inner.syscall_times[SYSCALL_CLOSE]+=1;drop(inner);return sys_close(args[0]);},
        ...
        SYSCALL_FSTAT => {inner.syscall_times[SYSCALL_FSTAT]+=1;drop(inner);return sys_fstat(args[0], args[1] as *mut Stat);},
